...
def _page_users(c):
    """Return the user list for page templates. Admins see everyone; non-admins only see themselves."""
    if _is_admin():
        return [row_to_dict(r) for r in c.execute("SELECT id, name, emoji, color, tfn, username, is_admin FROM users ORDER BY id").fetchall()]
...
def transactions_page():
    ...
    with get_db() as c:
        ...
        cats = c.execute("SELECT * FROM categories WHERE user_id IS NULL OR user_id=? ORDER BY type, name", (uid,)).fetchall()
        users = _page_users(c)
...
def api_get_users():
    ...
    if not _is_admin():
        for u in data:
            if u["id"] != caller:
                u["tfn"] = ""
    # Demo visitors must never see real household member names/usernames — only their own record.
    if caller == _DEMO_UID:
        data = [u for u in data if u["id"] == _DEMO_UID]
...
def api_add_user():
    ...
    if len(password) < 6: return jsonify({"ok":False,"error":"Password must be at least 6 characters"})
    with get_db() as c:
...
def api_update_user(uid):
    if not _is_admin():
        return jsonify({"ok": False, "error": "Forbidden"}), 403
    if uid == _DEMO_UID:
        return jsonify({"ok": False, "error": "Demo account cannot be modified"}), 403
    d = request.json
    with get_db() as c:
        if "tfn" in d:
...
def api_delete_user(uid):
    ...
    if uid == _DEMO_UID: return jsonify({"ok":False,"error":"Cannot delete demo account"})
    with get_db() as c:
...
def api_get_categories():
    ...
    with get_db() as c:
        if type_f:
            rows = c.execute(
                "SELECT * FROM categories WHERE type=? AND (user_id IS NULL OR user_id=?) ORDER BY name",
                (type_f, uid)).fetchall()
        else:
            rows = c.execute(
...
def api_add_category():
    ...
    if not name:
        return jsonify({"ok": False, "error": "Name is required"})
    # Admins add shared categories (user_id=NULL); non-admins add their own (user_id=self)
    owner = None if _is_admin() else session.get('user_id')
    with get_db() as c:
        try:
            cur = c.execute(
                "INSERT INTO categories (name, type, color, icon, user_id) VALUES (?,?,?,?,?)",
                (name, cat_type, d.get("color","#7c75f5"), d.get("icon",""), owner)
            )
            row = c.execute("SELECT * FROM categories WHERE id=?", (cur.lastrowid,)).fetchone()
            return jsonify({"ok": True, "data": row_to_dict(row)})
        except sqlite3.IntegrityError:
            row = c.execute("SELECT * FROM categories WHERE name=? AND type=? AND (user_id IS NULL OR user_id=?)",
                            (name, cat_type, session.get('user_id'))).fetchone()
...
def api_delete_category(cid):
    ...
    uid = session.get('user_id')
    with get_db() as c:
        row = c.execute("SELECT user_id FROM categories WHERE id=?", (cid,)).fetchone()
        if not row:
            return jsonify({"ok": False, "error": "Not found"}), 404
        # Non-admins can only delete their own categories, not shared ones
        if not _is_admin() and row["user_id"] != uid:
            return jsonify({"ok": False, "error": "Cannot delete a shared category"}), 403
...
def api_get_notes():
    ...
    with get_db() as c:
        rows = c.execute(
            "SELECT * FROM notes WHERE user_id=? ORDER BY pinned DESC, updated DESC", (uid,)
        ).fetchall()
    return jsonify({"ok": True, "data": [row_to_dict(r) for r in rows]})
...
def api_add_note():
    ...
    uid = session.get('user_id')
    with get_db() as c:
        count = c.execute("SELECT COUNT(*) as n FROM notes WHERE user_id=?", (uid,)).fetchone()["n"]
    if count >= 3:
...
def api_update_note(nid):
    ...
    d = request.json
    with get_db() as c:
        existing = c.execute("SELECT * FROM notes WHERE id=? AND user_id=?", (nid, uid)).fetchone()
        if not existing:
        ...
        c.execute(
            "UPDATE notes SET title=?, content=?, color=?, pinned=?, updated=datetime('now') WHERE id=? AND user_id=?",
            (title, content, color, pinned, nid, uid)
        )
